...
sealed interface MapScreenState {
    ...
    data class LineSelected(val line: Line, val selectedRoute: Route, val lineStops: List<Stop>?, val path: Path?) : MapScreenState, Dismissable
    data class LineStopSelected(
        val selectedStop: Stop,
        val lineSelectedState: LineSelected,
    ) : MapScreenState, Dismissable
