...
object Stubs {
    ...
    private fun randomCardSerial(): CardId = Random.nextLong(100000000, 999999999)

    @Deprecated("")
    val cardInfoTransactions = listOf(
        CardTransaction.Validation(randomCardSerial(), LocalDateTime.now(), 350, lines[4].toSummary(), bus = 0, people = 2),
        CardTransaction.Validation(randomCardSerial(), LocalDateTime.now(), 350, lines[2].toSummary(), bus = 0, people = 0),
        CardTransaction.TopUp(randomCardSerial(), LocalDateTime.now(), 10000),
        CardTransaction.Validation(randomCardSerial(), LocalDateTime.now(), 350, lines[34].toSummary(), bus = 0, people = 2),
        CardTransaction.Validation(randomCardSerial(), LocalDateTime.now(), 350, lines[34].toSummary(), bus = 0, people = 0),
    )

    @Deprecated("")
    val searchResults: List<SearchResult>
        get() {
            val stops = stops.shuffled().take(5)
                .map { SearchResult.StopResult(it) }
            val lines = lines.shuffled().take(3)
    ...
    val arrivals = listOf(
        ...
        BusArrival.NotAvailable(line = lines.random().toSummary(), route = routes.random())
    )
